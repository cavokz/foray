...
1. `types.rs`:
   ...
   - `validate_name()` for journal name validation — `[a-z0-9_-]`, non-empty, max **256** chars
2. `store.rs`:
   - `#[async_trait] trait Store: Send + Sync` with async methods: `load(name, pagination) -> (JournalFile, total)`, `create(name, title, meta)`, `add_items(name, &[JournalItem]) -> Result<Vec<String>, StoreError>` (returns IDs of items that failed to store), `list(pagination, archived) -> (Vec<JournalSummary>, total)`, `delete`, `exists`, `archive`, `unarchive`
...
3. `config.rs` — `StoreRegistry` and config parsing:
   ...
   - `nuance` is a FNV-1a hash of sorted fingerprints (`"name=path"` for json_file, `"name=command arg0 …"` for foray_stdio, `"name=url"` for elasticsearch) plus `"schema=N"` and `"protocol=N"` — deterministic, stable across restarts, changes when config, schema, or protocol version changes
...
6. `store_elasticsearch.rs` — `ElasticsearchStore`: HTTP client backed by `reqwest` (rustls-tls, no native-tls dependency). Stores journals and items as separate ES documents in a single index, distinguished by `event.dataset` (`"foray.journal"` / `"foray.item"`). Document IDs: `"journal:{name}"` and `"item:{item-id}"` (colon percent-encoded in URLs). All 8 `Store` trait methods map to ES REST API calls. ES field mapping: `@timestamp` (date/ECS), `message` (match_only_text/ECS), `tags` (keyword[]/ECS), `event.dataset` (keyword/ECS), `journal_name` (keyword, item docs only), `type` (keyword), `archived` (boolean, journal docs only), `entity.name`+`entity.display_name` (keyword+.text/ECS, journal docs only), `meta` (flattened). `create` uses `PUT _doc/{id}?op_type=create` — 409 → `AlreadyExists`. `add_items` bulk-indexes via `_bulk` with `op_type=create` (NDJSON), returns total item count from `_count`. `list` uses two requests: paginated journal search + terms aggregation for item counts. `delete` removes items first via `_delete_by_query` then the journal doc (items-first ensures the journal remains retryable if item deletion fails). `archive`/`unarchive` use `_update` with `{"doc": {"archived": true/false}}`. `exists` uses HEAD. ES errors are sanitised (no internal details exposed). Credentials never logged. Requires an operator-provisioned index template matching `foray-*` — see `doc/es-index-template.json`. Item pagination uses `_shard_doc` as sort tiebreaker (fielddata on `_id` is disabled by default). `item_count` returned by `list()` is approximate (NRT) by design — no `?refresh=wait_for` is added. Integration tests live in `tests/elasticsearch_store_test.rs` (14 tests, all `#[ignore = "requires ES_TEST_URL"]`); run with `cargo test --test elasticsearch_store_test -- --include-ignored` when `ES_TEST_URL` is set. A `docker-compose.yml` (ES 9, HTTP + credentials) and `Makefile` targets (`es-up`, `es-down`, `es-init`, `es-logs`) support local development. CI runs these tests in an `online-tests` job.
