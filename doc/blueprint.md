...
- `list_journals` → `{ journals: [{ name, title, item_count, avg_item_size?, std_item_size?, schema?, meta?, error? }], total }` (pass `archived: true` to list archived journals; `avg_item_size` and `std_item_size` are serialized JSON byte sizes — absent for empty journals or old servers; `schema` is the on-disk schema version — present whenever the file is parseable as a JSON object; `error` is present for journals that could not be fully loaded, in which case `title` is empty and `item_count` is 0)
...
1. `types.rs`:
   ...
   - `JournalSummary` { name, title, item_count, avg_item_size?, std_item_size?, schema?, meta?, error? } — `schema` is the on-disk schema version (present whenever the file is parseable as a JSON object); `avg_item_size` and `std_item_size` are the mean and population standard deviation of serialized item sizes in bytes (`None` when the journal has fewer than 2 items or the server does not support these fields); `error` is set for journals that could not be fully loaded (in which case `title` is empty and `item_count` is 0)
...
2. `store.rs`:
   - `#[async_trait] trait Store: Send + Sync` with async methods: `load(name, pagination) -> (JournalFile, total)`, `create(name, title, meta)`, `add_items(name, Vec<JournalItem>)`, `list(archived) -> (Vec<JournalSummary>, total)`, `delete`, `exists`, `archive`, `unarchive`
